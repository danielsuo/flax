...
flags.DEFINE_string('lr_schedule',
                    default='stepped',
                    help=('Learning rate schedule type; constant, stepped or cosine'))

flags.DEFINE_string('lr_sched_steps',
                    default='[[60, 0.2], [120, 0.04], [160, 0.008]]',
...
flags.DEFINE_float('l2_reg', default=0.0005, help=('The amount of L2-regularization to apply.'))

flags.DEFINE_integer('batch_size', default=256, help=('Batch size for training.'))

flags.DEFINE_integer('num_epochs', default=200, help=('Number of training epochs.'))

flags.DEFINE_string('arch', default='wrn26_10', help=('Network architecture'))

flags.DEFINE_float('wrn_dropout_rate', default=0.3, help=('Wide ResNet DropOut rate'))

flags.DEFINE_integer('rng', default=0, help=('Random seed for network initialization.'))
...
def create_model(prng_key, batch_size, image_size, module):
  ...
  with flax.nn.stateful() as init_state:
    with flax.nn.stochastic(jax.random.PRNGKey(0)):
      _, initial_params = module.init_by_shape(prng_key, [(input_shape, jnp.float32)])
...
def train(module,
  ...
  """Train model."""
  if jax.host_count() > 1:
    raise ValueError('CIFAR-10 example should not be run on ' 'more than 1 host (for now)')
  ...
  if make_lr_fun is None:
    ...
    def make_lr_fun(base_lr, steps_per_epoch):  # pylint: disable=function-redefined
      return lr_schedule.create_stepped_learning_rate_schedule(
          base_lr, steps_per_epoch, [[60, 0.2], [120, 0.04], [160, 0.008]])
  ...
  for step, batch in zip(range(num_steps), train_iter):
    ...
    # Train step
    optimizer, state, metrics = p_train_step(optimizer, state, batch, sharded_keys)
    ...
    if (step + 1) % steps_per_epoch == 0:
      ...
      # Log epoch summary
      logging.info('Epoch %d: TRAIN loss=%.6f, err=%.2f, EVAL loss=%.6f, err=%.2f', epoch,
                   train_summary['loss'], train_summary['error_rate'] * 100.0, eval_summary['loss'],
                   eval_summary['error_rate'] * 100.0)

      summary_writer.scalar('eval_loss', eval_summary['loss'], epoch)
      summary_writer.scalar('eval_error_rate', eval_summary['error_rate'], epoch)
...
def main(argv):
  ...
  if FLAGS.arch == 'wrn26_10':
    module = wideresnet.WideResnet.partial(blocks_per_group=4,
    ...
    module = wideresnet_shakeshake.WideResnetShakeShake.partial(blocks_per_group=4,
  ...
  if FLAGS.lr_schedule == 'constant':
    ...
    def make_lr_fun(base_lr, steps_per_epoch):
      return lr_schedule.create_constant_learning_rate_schedule(base_lr, steps_per_epoch)
    ...
    def make_lr_fun(base_lr, steps_per_epoch):
      return lr_schedule.create_cosine_learning_rate_schedule(base_lr, steps_per_epoch,
                                                              FLAGS.num_epochs)
  else:
    raise ValueError('Unknown LR schedule type {}'.format(FLAGS.lr_schedule))

  train(module, FLAGS.model_dir, FLAGS.batch_size, FLAGS.num_epochs, FLAGS.learning_rate,
