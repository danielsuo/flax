...
def bin_and_batch(dataset,
  ...
  if buckets is None:
    ...
    bucket_boundaries = [
        bucket_length // 4, bucket_length // 2, bucket_length, bucket_length * 2, bucket_length * 4,
        bucket_length * 8, bucket_length * 16
    ]
    bucket_batch_sizes = [
        batch_size * 4, batch_size * 2, batch_size, batch_size // 2, batch_size // 4,
  ...
  def example_length(example):
    ...
    return tf.maximum(tf.shape(example['inputs'])[0], tf.shape(example['targets'])[0])

  boundaries, batch_sizes = buckets
  # bucket_by_sequence_length expects a final dummy 1 batch_size.
  batch_sizes.append(1)
  dataset = dataset.apply(
      tf.data.experimental.bucket_by_sequence_length(example_length,
                                                     boundaries,
                                                     batch_sizes,
...
def get_wmt_datasets(n_devices,
  ...
  train_data = tfds.load(name=dataset_name,
                         split=tfds.Split.TRAIN,
                         data_dir=data_dir,
                         shuffle_files=True)

  eval_data = tfds.load(name=dataset_name,
  ...
  to_features_dict = (lambda x: {
      'inputs': x[features_info.supervised_keys[0]],
  ...
  try:
    ...
    subword_tokenizer = tfds.features.text.SubwordTextEncoder.build_from_corpus(
        corpus_generator(), target_vocab_size=target_vocab_size, max_corpus_chars=max_corpus_chars)
    # save vocab
  ...
  def encode(lang1, lang2):
    res1 = subword_tokenizer.encode(lang1.numpy()) + [subword_tokenizer.vocab_size]  # Add EOS Token
    res2 = subword_tokenizer.encode(lang2.numpy()) + [subword_tokenizer.vocab_size]  # Add EOS Token
    return res1, res2
