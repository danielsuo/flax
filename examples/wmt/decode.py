...
def beam_init(batch_size, beam_size, max_decode_len, cache):
  ...
  live_logprobs0 = jnp.tile(jnp.array([0.0] + [NEG_INF] * (beam_size - 1)), [batch_size, 1])
...
def beam_search(inputs,
  ...
  def beam_search_loop_cond_fn(state):
    ...
    worst_finished_scores = jnp.where(state.finished_flags, worst_finished_scores, NEG_INF)
  ...
  def beam_search_loop_body_fn(state):
    ...
    # {[batch * beam, ...], ...} --> {[batch, beam, ...], ...}
    new_cache = jax.tree_map(lambda x: unflatten_beam_dim(x, batch_size, beam_size), new_flat_cache)
    ...
    topk_seq, new_cache = gather_beams([state.live_seqs, new_cache], topk_beam_indices, batch_size,
                                       beams_to_keep)
    ...
    top_alive_seq, top_alive_log_probs, top_alive_cache = gather_topk_beams(
        [topk_seq, new_log_probs, new_cache], new_log_probs, batch_size, beam_size)
    ...
    top_finished_seq, top_finished_scores, top_finished_flags = (gather_topk_beams(
        [finished_seqs, finished_scores, finished_flags], finished_scores, batch_size, beam_size))
  ...
  final_state = lax.while_loop(beam_search_loop_cond_fn, beam_search_loop_body_fn,
  ...
  # --> [batch, beams, length]
  finished_seqs = jnp.where(none_finished[:, None, None], final_state.finished_seqs,
                            final_state.live_seqs)
  # --> [batch, beams]
  finished_scores = jnp.where(none_finished[:, None], final_state.finished_scores,
