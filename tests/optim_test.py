...
class ModelParamTraversalTest(absltest.TestCase):
  ...
  def test_param_selection(self):
    ...
    self.assertEqual(set(names), set(['/x/kernel', '/x/bias', '/x/y/kernel', '/x/y/bias']))
...
class MultiOptimizerTest(absltest.TestCase):
  def test_multi_optimizer(self):
    ...
    expected_hyper_params = [_GradientDescentHyperParams(1.), _GradientDescentHyperParams(10.)]
    self.assertEqual(optimizer_def.hyper_params, expected_hyper_params)
    expected_state = [optim.OptimizerState(0, [()])] * 2
    self.assertEqual(state, expected_state)
    grads = {'a': -1., 'b': -2.}
    new_params, new_state = optimizer_def.apply_gradient(optimizer_def.hyper_params, params, state,
                                                         grads)
...
class MomentumTest(absltest.TestCase):
  ...
  def test_apply_gradient(self):
    optimizer_def = optim.Momentum(learning_rate=0.1, beta=0.2)
    params = onp.ones((1, ))
    state = optim.OptimizerState(0, _MomentumParamState(onp.array([1.])))
    grads = onp.array([3.])
    new_params, new_state = optimizer_def.apply_gradient(optimizer_def.hyper_params, params, state,
                                                         grads)
    expected_new_state = optim.OptimizerState(1, _MomentumParamState(onp.array([3.2])))
...
class AdamTest(absltest.TestCase):
  def test_init_state(self):
    params = onp.zeros((1, ))
    optimizer_def = optim.Adam(learning_rate=0.1, beta1=0.2, beta2=0.9, eps=0.01, weight_decay=0.0)
    state = optimizer_def.init_state(params)

    expected_hyper_params = _AdamHyperParams(0.1, 0.2, 0.9, 0.01, 0.0)
    self.assertEqual(optimizer_def.hyper_params, expected_hyper_params)
    expected_state = optim.OptimizerState(0, _AdamParamState(onp.zeros((1, )), onp.zeros((1, ))))
    self.assertEqual(state, expected_state)

  def test_apply_gradient(self):
    optimizer_def = optim.Adam(learning_rate=0.1, beta1=0.2, beta2=0.9, eps=0.01, weight_decay=0.0)
    params = onp.array([1.])
    state = optim.OptimizerState(1, _AdamParamState(onp.array([0.1]), onp.array([0.9])))
    grads = onp.array([4.])
    new_params, new_state = optimizer_def.apply_gradient(optimizer_def.hyper_params, params, state,
                                                         grads)
    expected_new_state = optim.OptimizerState(2,
                                              _AdamParamState(onp.array([3.22]), onp.array([2.41])))
...
class AdagradTest(absltest.TestCase):
  ...
  def test_apply_gradient(self):
    ...
    new_params, new_state = optimizer_def.apply_gradient(optimizer_def.hyper_params, params, state,
                                                         grads)
    expected_new_state = optim.OptimizerState(2, _AdagradParamState(onp.array([16.1])))
...
class RMSPropTest(absltest.TestCase):
  def test_init_state(self):
    ...
    self.assertEqual(optimizer_def.hyper_params, expected_hyper_params)
    expected_state = optim.OptimizerState(0, _RMSPropParamState(onp.zeros((1, ))))
    self.assertEqual(state, expected_state)
  ...
  def test_apply_gradient(self):
    ...
    new_params, new_state = optimizer_def.apply_gradient(optimizer_def.hyper_params, params, state,
                                                         grads)
    expected_new_state = optim.OptimizerState(2, _RMSPropParamState(onp.array([1.69])))
